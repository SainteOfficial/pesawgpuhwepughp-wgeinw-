-- KeySystem laden
local KeySystem = loadstring(readfile("MythKeySystem.lua"))()

-- Startmeldung anzeigen
local notification = loadstring(game:HttpGet("https://tree-hub.vercel.app/api/UI/Notification"))()
notification:Notify({
    Title = "MythHub wird geladen",
    Content = "Initialisiere MythHub v3.5 - Bitte warten...",
    Duration = 3,
    Icon = "loader",
    Color = "Blue"
})

-- Fehlerbehandlung für den Startprozess
local function HandleError(errorMsg)
    notification:Notify({
        Title = "Fehler",
        Content = errorMsg or "Ein unbekannter Fehler ist aufgetreten.",
        Duration = 6,
        Icon = "alert-triangle",
        Color = "Red"
    })
    
    -- Log für Fehlerbehebung
    warn("[MythHub] Fehler: " .. (errorMsg or "Unbekannter Fehler"))
    
    -- Warten und erneut versuchen, das Skript zu laden
    task.delay(5, function()
        notification:Notify({
            Title = "MythHub",
            Content = "Versuche erneut zu laden...",
            Duration = 3,
            Icon = "refresh-cw",
            Color = "Orange"
        })
        
        -- Starte den Skriptladevorgang erneut
        loadstring(readfile("MythHub.lua"))()
    end)
end

-- Script erst nach erfolgreicher Key-Validierung starten
local success, errorMsg = pcall(function()
    KeySystem.ShowKeyUI(function(success)
        if success then
            -- Erfolgsmeldung
            notification:Notify({
                Title = "MythHub gestartet",
                Content = "Willkommen bei MythHub! Die Benutzeroberfläche wird geladen.",
                Duration = 3,
                Icon = "star",
                Color = "Green"
            })
...
-- Fehlerbehandlung für den Fall, dass das Skript fehlschlägt
if not success then
    HandleError(tostring(errorMsg))
end